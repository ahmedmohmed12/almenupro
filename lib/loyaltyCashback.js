...
function redeemCustomerWallet(customers, { phone, restaurantId, amount, orderId }) {
  const requested = roundCashback(amount);
  if (requested <= 0 || !phone) {
    return { customers, redeemed: 0 };
  }
  ...
  const currentBalance =
    Number(existing.walletBalance ?? existing.wallet_balance ?? 0) || 0;
  const redeemed = roundCashback(Math.min(currentBalance, requested));
  if (redeemed <= 0) {
    return { customers, redeemed: 0 };
  }
  ...
  next[existingIndex] = {
    ...existing,
    walletBalance: roundCashback(currentBalance - redeemed),
    wallet_balance: roundCashback(currentBalance - redeemed),
    walletHistory: history.slice(0, 50),
    wallet_history: history.slice(0, 50),
    updatedAt: now,
    updated_at: now,
  ...
  return { customers: next, redeemed };
}
