...
function attachAcceptedBy(order, previous, auth = {}, body = {}, persistedStatus) {
  const prev = String(previous?.status || '').toLowerCase();
  const next = String(persistedStatus || order.status || '').toLowerCase();
  const isAccept =
    (prev === 'pending' || prev === '') &&
    (next === 'confirmed' || next === 'preparing' || next === 'ready');
  if (!isAccept) return order;

  const existingId = firstNonEmpty(
    order.acceptedBy,
    order.accepted_by,
    previous?.acceptedBy,
    previous?.accepted_by,
  );
  if (existingId) {
    return {
      ...order,
      acceptedBy: existingId,
      accepted_by: existingId,
      acceptedByName: firstNonEmpty(
        order.acceptedByName,
        order.accepted_by_name,
        previous?.acceptedByName,
        previous?.accepted_by_name,
      ),
      acceptedAt: order.acceptedAt || order.accepted_at || previous?.acceptedAt || new Date().toISOString(),
    };
  }
  ...
  const acceptedBy = firstNonEmpty(
    ...
    body.acceptedBy,
    body.accepted_by,
    auth.role === 'restaurant_admin' ? `admin:${auth.restaurantId || ''}` : '',
    auth.role === 'super_admin' ? 'super_admin' : '',
    auth.role,
  );
  const acceptedByName = firstNonEmpty(
    auth.staffName,
    auth.staff_name,
    body.acceptedByName,
    body.accepted_by_name,
    auth.restaurantName,
    acceptedBy,
  );
  ...
  return {
    ...
    acceptedAt: order.acceptedAt || order.accepted_at || new Date().toISOString(),
  };
}
