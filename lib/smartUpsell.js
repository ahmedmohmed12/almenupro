...
function computeSmartUpsell({
  ...
  const threshold = Number(dessertThreshold) > 0
    ? Number(dessertThreshold)
  ...
  if (enabled === false) {
    return {
      ...
      dessertThreshold: threshold,
      gaps: { needsDrink: false, needsSide: false, needsDessert: false },
      recommendations: [],
      suggestedItems: [],
    };
  }
  ...
  const gaps = {
    needsDrink: suggestDrinks !== false && hasMain && !hasDrink,
    needsSide: suggestSides !== false && hasMain && !hasSide,
    needsDessert:
      suggestDesserts !== false && total >= threshold && !hasDessert,
  };
  ...
  if (gaps.needsDessert) {
    pushGroup(
      'dessert',
      'السلة فوق ' + threshold.toFixed(3) + ' د.ك — اقترح حلويات',
      'Cart is over ' + threshold.toFixed(3) + ' KD — suggest a dessert',
